...
class JsonFileStorage(StorageBackend):
    ...
    Examples:
        ...
        ...     storage.close()
    """
    SOFT_LIMIT_MB = 10  # large payload warning
    ...
    def _save(self, payload: Dict[str, Any]) -> None:
        ...
        # --- Added large payload warning ---
        data_size_mb = len(data.encode("utf-8")) / (1024 * 1024)
        if data_size_mb > self.SOFT_LIMIT_MB:
            warnings.warn(
                f"Saving large JSON payload ({data_size_mb:.2f} Mb) to {self.path}. "
                "Consider switching to a database storage.",
                UserWarning,
                stacklevel=2,
            )
